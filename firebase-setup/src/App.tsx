...
export default function App() {
  ...
  // Firestore real-time listeners manage all state synchronization
  ...
  // Master Firestore real-time synchronizer (Real-time live updater across all devices)
  useEffect(() => {
    const unsubscribers: (() => void)[] = [];

    // Subscribe to all data collections in real-time
    ...
    unsubscribers.push(listenToDocument('gallery', setGallery));
    unsubscribers.push(listenToDocument('videos', setVideos));
    unsubscribers.push(listenToDocument('news', setNews));
    unsubscribers.push(listenToDocument('priceList', setPriceList));
    unsubscribers.push(listenToDocument('contact', setContact));
    unsubscribers.push(listenToDocument('settings', setSettings));
    unsubscribers.push(listenToDocument('bookings', setBookings));
    unsubscribers.push(listenToDocument('testimonials', setTestimonials));
  ...
  const handleAddBooking = async (request: BookingRequest) => {
    ...
    const updatedBookings = [request, ...bookings];
    setBookings(updatedBookings);

    // 2. Real-time Firestore sync - saves to database so all devices see it instantly
    try {
      await saveDocument('bookings', updatedBookings);
    } catch (err) {
      console.error("Failed to persist booking to Firebase:", err);
    }
  };
